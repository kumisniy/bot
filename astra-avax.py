...
AA=C.to_checksum_address('0xB31f66AA3C1e785363F0875A1B74E27b85FD66c7')
j=C.to_checksum_address('0xb97ef9ef8734c71904d8002f8b6bc66dd9c48a6e')
U=f(L(A_+'erc20.abi'))
V=C.eth.contract(address=S.to_checksum_address('0x60aE616a2155Ee3d9A68541Ba4544862310933d4'),abi=f(L(A_+'router.abi')))
BI=C.eth.contract(address=S.to_checksum_address('0x9Ad6C38BE94206cA50bb0d90783181662f0Cfa10'),abi=f(L(A_+'factory.abi')))
...
def C3(token_address,amt=BG):A=S.to_checksum_address(token_address);B=C.eth.contract(address=A,abi=U);D=B.functions.allowance(G,V.address).call();return D>=amt
def C4(token_address,amt=BG,timeout=900):A('Approving token');B=C.eth.gasPrice;D=S.to_checksum_address(token_address);E=C.eth.contract(address=D,abi=U);F=E.functions.approve(V.address,amt);H={AR:G,AS:B,AT:C.eth.getTransactionCount(G)};I=F.buildTransaction(H);J=C.eth.account.sign_transaction(I,private_key=W);K=C.eth.sendRawTransaction(J.rawTransaction);C.eth.waitForTransactionReceipt(K,timeout=timeout)
def C5():
...
a=x
AB=x
def BQ():global t;global G;global W;global I;global Af;global AD;global AE;global Ag;global Ah;global Ai;t=m.get();G=S.to_checksum_address(b.get());W=A4.get();I=S.to_checksum_address(X.get());Af=p.get();AD=o.get();AE=n.get();Ag=q.get();Ah=r.get();Ai=s.get()
AC(AU)
B.mainloop()
